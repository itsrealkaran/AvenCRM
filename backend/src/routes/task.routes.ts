...
router.post('/', async (req: Request, res: Response) => {
  const { title, description, priority, status, dueDate, colorTag, tags, category } = req.body;
...
router.delete('/', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (tasks.length !== taskIds.length) {
      return res.status(403).json({ message: 'Some tasks not found or unauthorized' });
    }
