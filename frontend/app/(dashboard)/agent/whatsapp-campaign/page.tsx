  ...
  return (
    <Card className='p-6 space-y-6 min-h-full'>
      <div className="flex justify-between items-center">
        ...
        {!isConnected ? (
          <Button onClick={() => setShowWhatsAppModal(true)} className="bg-[#25D366] hover:bg-[#25D366]/90 text-white">
            <FaWhatsapp className="w-4 h-4 mr-2" />
            Connect WhatsApp
          </Button>
        ) : (
          <Button onClick={() => setShowCampaignModal(true)} className="bg-[#5932EA] hover:bg-[#5932EA]/90">
            Create Campaign
          </Button>
        )}
      </div>
