...
const GraphComp: React.FC<val> = ({ width, text, Comp, padding }) => {
  return (
    <>
      <div
        style={{ width: width + '%' }}
        className={` h-full overflow-hidden px-${padding} rounded-xl pt-2 bg-white  `}
      >
        <div className='w-full mt-1 px-4 flex justify-between items-center'>
          ...
          <select
            className='outline-none bg-[#F8F8FF] px-2 py-1 rounded-xl text-xs opacity-80'
            name='cars'
            id='cars'
          >
        ...
        <div className='w-full h-[85%]  '>
          <div className='w-full h-full  scale-[90%]'>{Comp && <Comp />}</div>
        </div>
      </div>
    </>
  );
