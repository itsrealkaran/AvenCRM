...
const StatsOverview: React.FC<val> = ({
  Cname,
  plan,
  date,
  assignee,
  ...
  return (
    <>
      <div
        onClick={func}
        className='w-full cursor-pointer flex-shrink-0  flex items-center justify-between py-1 bg-[#f5f5f5] rounded-xl '
      >
      ...
      {open ? (
        <div className='absolute top-0 left-0 w-full h-screen overflow-hidden bg-white '>
          <button className='border-[1px] border-black' onClick={() => func()}>
            go back
          </button>
        </div>
      ) : (
