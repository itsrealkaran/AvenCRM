...
const Menu: React.FC<MenuProps> = ({ icons: Icon, heading, reff, status }) => {
  ...
  return (
    <Link
      href={reff}
      className={`w-full flex justify-between  py-[10px] rounded-[9px] pl-3 pr-[5px] items-center ${status ? 'bg-[#5932ea] text-white' : 'opacity-45'} hover:bg-[#5932ea]  hover:opacity-100 hover:text-white  h-fit`}
    >
