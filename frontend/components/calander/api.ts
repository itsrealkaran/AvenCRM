...
export const createEvent = async (eventData: any) => {
  const response = await api.post(`${process.env.NEXT_PUBLIC_BACKEND_URL}/calender/createEvent`,
    eventData, {
      headers: {
        Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
      }
...
export const deleteEvent = async (id: any) => {
  const response = await api.delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/calender/delete/${id}`, {
    headers: {
      Authorization: `Bearer ${localStorage.getItem('accessToken')}`,
    }
  });
