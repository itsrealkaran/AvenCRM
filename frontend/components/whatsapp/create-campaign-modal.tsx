...
interface CreateCampaignModalProps {
  ...
  onCreateAudience: (audience: AudienceGroup) => AudienceGroup // Updated prop type
...
const SAMPLE_TEMPLATES = [
  { id: "template1", name: "Welcome Message", content: "Welcome, {{1}}! We're glad to have you on board." },
  { id: "template2", name: "Order Confirmation", content: "Your order #{{1}} has been confirmed. Total: {{2}}" },
]
...
export function CreateCampaignModal({
  ...
  useEffect(() => {
    if (editingCampaign) {
      ...
      setSelectedTemplate(SAMPLE_TEMPLATES.find((t) => t.id === editingCampaign.templateId) || SAMPLE_TEMPLATES[0])
  ...
  const validateForm = () => {
    ...
    if (campaignType === "template" && Object.values(templateParams).some((param) => !param.trim())) {
          ...
          <div className="space-y-4">
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={(value) =>
                  setSelectedTemplate(SAMPLE_TEMPLATES.find((t) => t.id === value) || SAMPLE_TEMPLATES[0])
                }
              >
                <SelectTrigger>
            ...
            {selectedTemplate.content.match(/{{(\d+)}}/g)?.map((param, index) => (
              <div key={index} className="space-y-2">
                ...
                <Input
                  ...
                  onChange={(e) => setTemplateParams({ ...templateParams, [`${index + 1}`]: e.target.value })}
                />
              </div>
            ))}
            {errors.templateParams && <p className="text-sm text-red-500">{errors.templateParams}</p>}
          </div>
        )
      default:
        return null
  ...
  const renderMessagePreview = () => {
    ...
    switch (campaignType) {
      case "text":
        previewContent = <p className="text-sm break-words">{message || "Your message preview will appear here"}</p>
        break
      case "image":
        previewContent = (
          <div className="space-y-2">
            <img src={imageUrl || "/placeholder.svg"} alt="Preview" className="max-w-full h-auto rounded" />
      ...
      <Card className="bg-gray-50">
        <CardContent className="p-6">
          ...
          <div className="bg-white rounded-lg p-4 shadow-sm">
            ...
            <div className="bg-[#DCF8C6] rounded-lg p-3 inline-block max-w-full">{previewContent}</div>
          </div>
        </CardContent>
      </Card>
    )
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Select
                  ...
                  onValueChange={(value) => setSelectedAudience(audiences.find((a) => a.id === value) || null)}
                >
...
const ImageSelectionModal = ({ handleImageSelection }: { handleImageSelection: (imageData: string) => void }) => {
  return (
    <DialogContent className="sm:max-w-[425px]">
      <DialogHeader>
        <DialogTitle>Select Image</DialogTitle>
        <DialogDescription>Choose an image from your CRM or upload from your device.</DialogDescription>
      </DialogHeader>
