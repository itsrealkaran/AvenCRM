...
import { Plus, Trash2, GripVertical, ChevronDown, ChevronUp, Home } from "lucide-react"
...
const Step4Rooms: React.FC = () => {
  const { formData, addFloor, removeFloor, updateFloor, addRoom, updateRoom, removeRoom, reorderFloors, toggleMetric } =
    usePropertyForm()
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center space-x-2">
          <span className={formData.isMetric ? "text-muted-foreground" : "font-medium"}>Imperial</span>
          <Switch
          ...
          />
          <span className={!formData.isMetric ? "text-muted-foreground" : "font-medium"}>Metric</span>
        </div>
      </div>
            ...
            <Card key={floor.id} className="relative group">
              ...
              <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
                ...
                <div className="flex items-center gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => addRoom(floor.id)}>
                    <Plus className="w-4 h-4 mr-2" />
                    Add Room
                  </Button>
                  <Button type="button" variant="ghost" size="sm" onClick={() => removeFloor(floor.id)}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <Card key={room.id} className="bg-muted/50">
                      <CardContent className="p-4 space-y-4">
                        <div className="flex items-center justify-between gap-2">
                          ...
                          <Select
                            value={room.type}
                            onValueChange={(value) => updateRoom(floor.id, room.id, { type: value })}
                          >
                        ...
                        <div className="grid grid-cols-2 gap-2">
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              step="0.01"
                              value={room.width}
                              onChange={(e) => updateRoom(floor.id, room.id, { width: e.target.value })}
                          ...
                          <div className="space-y-1">
                            ...
                            <Input
                              ...
                              step="0.01"
                              value={room.length}
                              onChange={(e) => updateRoom(floor.id, room.id, { length: e.target.value })}
                              placeholder={formData.isMetric ? "Meters" : "Feet"}
                              className="h-8"
                            />
                          </div>
                  ...
                  {floor.rooms.length === 0 && (
                    <div className="col-span-2 flex flex-col items-center justify-center p-8 border-2 border-dashed rounded-lg">
                      ...
                      <p className="text-sm text-muted-foreground">No rooms added to this floor yet</p>
                      <Button
                        type="button"
                        variant="outline"
                        size="sm"
